...
//
//        for (
//          _ <- GameValidator.checkPlayersNumber(playersSet).leftMap(CompetioBriscolaError(_));
//          players <- GameValidator.checkAllPlayersExists(playerById, playersSet).leftMap(CompetioBriscolaError(_))
//        ) yield {
//          val issr = players.find(_.id == issuer).get
//          Seq(CreatedCompetition(nextId, issr, Competition(gamePlayers, kind, deadLine)))
//        }
