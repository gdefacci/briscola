...
object Step {
  ...
  implicit val functor: Functor[Step] = new Functor[Step] {
    def map[A,B](step: Step[A])(f: A => B): Step[B] = step match {
      ...
      case GetSiteMap(next) => GetSiteMap(next andThen f)
  ...
  def getSiteMap:FreeStep[SiteMap] = liftF( GetSiteMap(identity) )
...
case class GetSiteMap[N](next:SiteMap => N) extends Step[N]
